...
const EventForm = ({ userId, type, event, eventId }: EventFormProps) => {
  ...
  async function onSubmit(values: z.infer<typeof eventFormSchema>) {
    let uplaodImageUrl = values.imageUrl;

    if (files.length > 0) {
      const uplaodImages = await startUpload(files);

      if (!uplaodImages) {
        return;
      }

      uplaodImageUrl = uplaodImages[0].url;
    }

    if (type === "Create") {
      try {
        const newEvent = await createEvent({
          event: { ...values, imageUrl: uplaodImageUrl },
          userId,
          path: "/profile",
        });
        console.log(userId);
    ...
    if (type === "Update") {
      ...
      try {
        const updatedEvent = await updateEvent({
          userId,
          event: { ...values, imageUrl: uplaodImageUrl, _id: eventId },
          path: `/events/${eventId}`,
        });
        console.log(userId);
